...
public final class BinaryTreeMethods {
    ...
    public static <T> int height(BinaryTree<T> t) {
        ...
        if (t.size() != 0) {
            ...
            max = Math.max(max, leftHeight);
            max = Math.max(max, rightHeight);
            height += max;
    ...
    public static <T> boolean isInTree(BinaryTree<T> t, T x) {
        ...
        if (t.size() > 0) {
            answer = t.root().equals(x);
            BinaryTree<T> lhs = new BinaryTree1<>();
            BinaryTree<T> rhs = new BinaryTree1<>();
            T temp = t.disassemble(lhs, rhs);
            boolean left = isInTree(lhs, x);
            boolean right = isInTree(rhs, x);
            t.assemble(temp, lhs, rhs);
