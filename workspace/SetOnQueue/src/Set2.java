...
public class Set2<T> extends SetSecondary<T> {
    ...
    private static <T> void moveToFront(Queue<T> q, T x) {
        ...
        Queue<T> toBeTransfered = q.newInstance();
        boolean contains = false;

        for (int i = 0; i < q.length(); i++) {
            T temp = q.dequeue();
            if (temp.equals(x)) {
                toBeTransfered.enqueue(temp);
                i--;
            } else {
                q.enqueue(temp);
            }
        ...
        toBeTransfered.append(q);
        q.transferFrom(toBeTransfered);
    }
    ...
    public final T remove(T x) {
        ...
        Queue<T> transfer = this.elements.newInstance();
