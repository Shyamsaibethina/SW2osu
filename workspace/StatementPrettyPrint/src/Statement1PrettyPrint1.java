        ...
        switch (this.kind()) {
            case BLOCK: {
                ...
                for (int i = 0; i < length; i++) {
                    Statement childBlock = this.removeFromBlock(i);
                    childBlock.prettyPrint(out, offset);
                    this.addToBlock(i, childBlock);
                }
                break;
            }
            case IF: {

                Statement childBlock = this.newInstance();
                Statement.Condition ifCondition = this
                        .disassembleIf(childBlock);
                printSpaces(out, offset);

                out.println("IF " + toStringCondition(ifCondition) + " THEN");
                childBlock.prettyPrint(out, offset + indent);

                for (int i = 0; i < offset; i++) {
                    out.print(" ");
                }
                out.println("END IF");
                this.assembleIf(ifCondition, childBlock);
                break;
            }
            case IF_ELSE: {

                Statement childBlockIf = this.newInstance();
                Statement childBlockElse = this.newInstance();
                Condition ifElseCondition = this.disassembleIfElse(childBlockIf,
                        childBlockElse);
                printSpaces(out, offset);
                out.println(
                        "IF " + toStringCondition(ifElseCondition) + " THEN");
                childBlockIf.prettyPrint(out, offset + indent);

                printSpaces(out, offset);
                out.println("ELSE");
                childBlockElse.prettyPrint(out, offset + indent);

                printSpaces(out, offset);
                out.println("END IF");
                this.assembleIfElse(ifElseCondition, childBlockIf,
                        childBlockElse);
            ...
            case WHILE: {
                ...
                Statement childBlock = this.newInstance();
                Condition whileCondition = this.disassembleWhile(childBlock);
                printSpaces(out, offset);
                out.println(
                ...
                this.assembleWhile(whileCondition, childBlock);
